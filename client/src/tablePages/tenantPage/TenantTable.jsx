...
import TenantCells from "./TenantCells";
...
export default function TenantTable({
  ...
  const getTaskStatus = (tenant) => {
    if (
      tenant.completedTasks > 0 &&
      tenant.completedTasks === tenant.totalTasks
    ) {
      return "Completed";
  ...
  const filteredTenants = (tenants || []).filter((tenant) => {
    const fullName = `${tenant.firstName ?? ""} ${
      tenant.lastName ?? ""
    }`.toLowerCase();
  ...
  const paginatedTenants = stableSort(
    filteredTenants,
