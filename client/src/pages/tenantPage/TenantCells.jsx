...
const TenantCells = ({
  ...
  >
      ...
      <tr>
        ...
        >
          <Checkbox
            ...
            color={
              (selected?.length || 0) > 0 ||
              (selected?.length || 0) === (tenants?.length || 0)
                ? "primary"
                : undefined
            }
    ...
    <tbody>
      {stableSort(paginatedTenants, getComparator(order, "id")).map(
        (tenant) => {
          ...
          return (
            <tr key={tenant._id}>
              ...
              {columnVisibility.address && (
                <td className="table-cell">{`${
                  tenant.address?.flat ? `${tenant.address.flat}, ` : ""
